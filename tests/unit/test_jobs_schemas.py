...
def test_job_response_valid_transition():
    ...
    job = JobResponse(job_id="123", status="PENDING")
    updated_job = JobResponse(job_id="123", status="RUNNING", previous_status=job.status)
    assert updated_job.status == "RUNNING"

    job = updated_job
    updated_job = JobResponse(job_id="123", status="COMPLETED", previous_status=job.status)
...
def test_job_response_invalid_transition():
    ...
    assert "Invalid state transition from 'COMPLETED' to 'RUNNING'" in str(exc_info.value)
