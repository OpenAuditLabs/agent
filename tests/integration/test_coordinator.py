...
async def test_orchestrator_coordinator_timeout():
    ...
    with (
        patch('oal_agent.agents.coordinator.CoordinatorAgent.route', new_callable=AsyncMock) as mock_route,
        ...
        with pytest.raises(OrchestrationError, match="Coordinator routing timed out for job timeout_job_456"):
            await orchestrator.orchestrate(job_id, job_data)
...
async def test_orchestrator_simulated_error():
    ...
    with pytest.raises(OrchestrationError, match="Failed to orchestrate job simulate_error"):
        await orchestrator.orchestrate(job_id, job_data)
